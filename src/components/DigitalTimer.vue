    ...
    <div class="w-full">
      <div class="flex flex-col items-center gap-4">
        ...
        >
          ...
          <Transition name="time-fade" mode="out-in">
            <span :key="menit" class="inline-block tabular-nums">{{
              menit
            }}</span>
          </Transition>
          <span class="opacity-60">:</span>
          <Transition name="time-fade" mode="out-in">
            <span :key="detik" class="inline-block tabular-nums">{{
              detik
            }}</span>
          </Transition>
        </div>
        <span
          class="text-xs uppercase tracking-[0.18em] text-neutral-500 dark:text-neutral-400 mt-2"
        >
          {{ formattedDate }}
        </span>
      </div>
